                                            ...
                                            @foreach($invoice->items as $key=>$item)
                                                @if($item->item_type == 'item')
                                                    <tr>
                                                        ...
                                                        <td class="hidden-xs"> <i class="fa {{ $gymSettings->currency->symbol }}"></i>{{ $item->cost_per_item }} </td>
                                                        <td> <i class="fa {{ $gymSettings->currency->symbol }}"></i>{{ $item->amount }} </td>
                                                    </tr>
                                                @endif
                                            @endforeach
                                            </tbody>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-xs-12 col-md-8 invoice-block">
                                        <ul class="list-unstyled amounts">
                                            <li>
                                                <strong>Sub - Total amount:</strong> <i class="fa {{ $gymSettings->currency->symbol }}"></i>{{ round($invoice->sub_total, 2) }}
                                            </li>
                                            @foreach($invoice->items as $key=>$item)
                                                @if($item->item_type != 'item')
                                                    <li>
                                                        <strong>@if($item->item_type == 'discount')Discount:@else {{ strtoupper($item->item_name) }}: @endif</strong> <i class="fa {{ $gymSettings->currency->symbol }}"></i>{{ round($item->amount, 2) }}
                                                    </li>
                                                @endif
                                            @endforeach

                                            <li>
                                                <strong>Grand Total:</strong> <i class="fa {{ $gymSettings->currency->symbol }}"></i>{{ round($invoice->total, 2) }}
                                            </li>
                                        </ul>
                                        <br/>
                                        @if($isDesktop)
